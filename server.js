...
// نکته‌ی مهم (رفع باگ): برخلاف نسخه‌ی قبلی، اینجا هندشیک واقعی WebSocket زودتر
// از زمانی که خودِ کد ورکر با new WebSocketPair() تصمیم به آپگرید می‌گیرد انجام
// نمی‌شود. تا وقتی ورکر یک WebSocketPair نساخته، هر Response معمولی (خطای ۴۰۱/۴۰۴
// و ...) که برگرداند مستقیماً به‌عنوان پاسخ HTTP واقعی روی همان سوکت خام نوشته
// می‌شود و اتصال بسته می‌شود — دقیقاً مثل Cloudflare Workers که در آن تا وقتی
// fetch() خودش Response با status:101 برنگرداند، هیچ آپگریدی در سطح شبکه رخ
// نمی‌دهد. این یعنی اتصال‌های بدون احراز هویت/آیدی نامعتبر دیگر «باز و بلاتکلیف»
// نمی‌مانند و واقعاً رد می‌شوند.
...
class WebSocketPair {
  constructor() {
    const store = wsALS.getStore();
    if (!store) throw new Error("WebSocketPair used outside of a WebSocket upgrade");
    if (store.pair) { this[0] = store.pair[0]; this[1] = store.pair[1]; return; }

    // تا وقتی هندشیک واقعی کامل نشده، send/close/addEventListener صف می‌شوند
    const pending = { sends: [], closes: [], listeners: { message: [], close: [], error: [] } };
    const wrapper = {
      accept() {},
      send(data) {
        if (store.rawWs) { try { store.rawWs.send(data); } catch {} }
        else pending.sends.push(data);
      },
      close(code, reason) {
        if (store.rawWs) { try { store.rawWs.close(code, reason); } catch {} }
        else pending.closes.push([code, reason]);
      },
      addEventListener(type, cb) {
        if (store.rawWs) { wrapRawWs(store.rawWs).addEventListener(type, cb); }
        else if (pending.listeners[type]) pending.listeners[type].push(cb);
      },
      removeEventListener(type, cb) {
        if (store.rawWs) { wrapRawWs(store.rawWs).removeEventListener(type, cb); return; }
        if (!pending.listeners[type]) return;
        const i = pending.listeners[type].indexOf(cb);
        if (i > -1) pending.listeners[type].splice(i, 1);
      },
    };
    Object.defineProperty(wrapper, "bufferedAmount", { get: () => (store.rawWs && store.rawWs.bufferedAmount) || 0 });

    store.pair = [wrapper, wrapper];
    store.committed = true; // از این لحظه، ورکر رسماً تصمیم به آپگرید گرفته
    store.wss.handleUpgrade(store.req, store.socket, store.head, (rawWs) => {
      store.rawWs = rawWs;
      const w = wrapRawWs(rawWs);
      for (const type of Object.keys(pending.listeners)) {
        for (const cb of pending.listeners[type]) w.addEventListener(type, cb);
      }
      for (const data of pending.sends) { try { rawWs.send(data); } catch {} }
      for (const [code, reason] of pending.closes) { try { rawWs.close(code, reason); } catch {} }
    });
...
// اگر ورکر تا انتهای fetch() هیچ WebSocketPair نساخت (یعنی رد شد)، پاسخ معمولی
// خودش (مثلاً JSON خطای ۴۰۱/۴۰۴) را مستقیم روی سوکت خام به‌عنوان HTTP بنویس و ببند.
function writeRawHttpResponse(socket, response) {
  try {
    ...
    else if (body instanceof Uint8Array) bodyBuf = Buffer.from(body);
    else bodyBuf = Buffer.from(String(body), "utf8");

    const lines = [`HTTP/1.1 ${status} ${statusText}`];
    let hasCT = false, hasCL = false;
    for (const [k, v] of response.headers.entries()) {
      lines.push(`${k}: ${v}`);
      const kl = k.toLowerCase();
      if (kl === "content-type") hasCT = true;
      if (kl === "content-length") hasCL = true;
    }
    if (!hasCT) lines.push("Content-Type: application/json; charset=utf-8");
    if (!hasCL) lines.push(`Content-Length: ${bodyBuf.length}`);
...
async function main() {
  ...
  server.on("upgrade", (req, socket, head) => {
    const store = { req, socket, head, wss, rawWs: null, pair: null, committed: false };
    wsALS.run(store, async () => {
      try {
        const fetchReq = buildFetchRequest(req, Buffer.alloc(0));
        const response = await worker.fetch(fetchReq, env);
        // اگر ورکر WebSocketPair نساخت (یعنی رد شد)، همان پاسخ واقعی را روی سوکت بنویس
        if (!store.committed) writeRawHttpResponse(socket, response);
      } catch (err) {
        console.error("WS upgrade error:", err);
        if (!store.committed) { try { socket.destroy(); } catch {} }
      }
    });
  });
